from text_proccesing.unique_counter import text_to_unique_words
from random import choice


# return random description of movie which rating is higher than entered by user.
def movie_description(user_rating):
    description_list = []
    for movie in imdb_rank:
        if float(movie["rating"]) >= user_rating:
            description_list.append(movie["description"])
    return choice(description_list)
...
user_rating = float(input("Please enter a movie rating - a float number between 0 and 9.3 >>> ").replace(",", "."))

if 0.0 <= user_rating <= 9.3:
    print(movie_description(user_rating))
else:
    print("There is no movies with such high rating. Please enter a float number between 0 and 9.3")

print(text_to_unique_words(movie_description(user_rating)))
