...
cities = (('Ivano-Frankivsk', 'Ukraine'), ('Jakarta', 'Indonesia'),
          ...
          ('Lviv', 'Ukraine'), ('Odesa', 'Ukraine'), ('Sumy', 'Ukraine'),
          ('Lima', 'Peru'), ('London', 'United Kingdom'),
          )


# def ukr_cities(cities_list):
#     if cities_list[1] == 'Ukraine':
#         return cities_list[0]

def ukr_cities(cities_list):
    return 'Ukraine' in cities_list


ukrainian_only = filter(ukr_cities, cities)
print(set(ukrainian_only))

# def ukr_cities(cities_list):
#     city, country = cities_list[0]
#     if country == 'Ukraine':
#         return city
...
# Завдання 2
# Створіть функцію котра приймає текст(story) як аргумент, та повертає СЛОВНИК в котрому ключі це унікальні слова з тексту, а їх значення - кількість входження слова в текст.
# Тобто скільки разів у тексті зустрічається те чи інше слово.
...
print(text_to_dict(story))

# Завдання 3
# За допомогою lambda функції відсортуйте список що містить оцінки з предметів від найвищого балу до найменшого.

grade = [('Англійська мова', 88), ('Біологія', 90), ('Математика', 97), ('Українська мова', 82)]

sorted_grades = sorted(grade, key=lambda subject: subject[1], reverse=True)
print(sorted_grades)


# другий способ без лямбди
def func_name_not_lamb(x):
    return x[1]


sorted_grades = sorted(grade, key=func_name_not_lamb, reverse=True)
print(sorted_grades)


# Завдання 4
# За допомогою lambda функції та вбудованої функції filtered - відфільтруй назви
# фільмів рейтинг яких вище за 8.5 зі змінної imdb_rank, а після відсортуй за
# алфавітом по title.
...
filtered_movies = filter(lambda movie: float(movie["rating"]) > 8.5, imdb_rank)
sorted_filtered_movies = sorted(filtered_movies, key=lambda movie: movie["title"])

for movie in sorted_filtered_movies:
  print(movie["title"])


# Завдання 5
# Створіть функцію котра приймає назву фільму - знаходить його у змінній imdb_rank та повертає посилання на трейлер фільма.


# def trailer_search(movie_title: str) -> str:
#   for movie in imdb_rank:
#     if movie["title"] == movie_title:
#       return movie["trailer"]
#
#
# print(trailer_search(input("Please enter a movie title to get a trailer >>> ")))


# Завдання 6
# Створіть функцію котра має 1 параметр та отримує ім'я writers або director
# та поверне словник з іменем сценариста або режисера, кількіст його фільмів,
# місце в рейтингу та назви цих фільмів зі змінної


def participate_in_movies(writer_director_name: str) -> dict:
    movies_list = []
    info_dict = {"name": writer_director_name, "movies_quantity": 0, "movies_list": movies_list}
    for movie in imdb_rank:
        if writer_director_name in movie["director"] or writer_director_name in movie["writers"]:
          movie_info = {"title": movie["title"], "rank": movie["rank"]}
          movies_list.append(movie_info)
          info_dict["movies_quantity"] += 1
    return info_dict


print(participate_in_movies("Christopher Nolan"))



# Завдання 7
# Створіть функцію котра приймає назву жанку(ganre) зі змінної imdb_rank
# (попередньо підготуйте список унікальних жанрів) та
# повертає список фільмів цього жанру.

genre_name_list = []
for movie in imdb_rank:
  genre_name_list.extend(movie["genre"])

unique_genre_list = sorted(list(set(genre_name_list)))
# print(len(genre_name_list), genre_name_list)
print("The unique genre list is",  unique_genre_list)

def movie_genre(genre_name: str) -> list:
  same_genre_list = []
  for movie in imdb_rank:
    if genre_name in movie["genre"]:
      same_genre_list.append(movie["title"])
  return same_genre_list


print("The movies in genre", unique_genre_list[0], "are", movie_genre(unique_genre_list[0]))
